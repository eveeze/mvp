...
class ProcessVideoUpload implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        try {
            ...
            // [BARU] 3.5 Generate Thumbnail (Snapshot detik ke-1)
            $localThumbPath = $tempDir . '/thumbnail.jpg';
            $thumbCmd = [
                'ffmpeg', '-y', '-i', $localRawPath,
                '-ss', '00:00:01.000',
                '-vframes', '1',
                '-vf', 'scale=480:-1', // Lebar 480px, tinggi auto
                $localThumbPath
            ];
            (new Process($thumbCmd))->run();


            // 4. Susun Command FFmpeg (TETAP SAMA: 4 KUALITAS + AUDIO FIX)
            $cmd = ['ffmpeg', '-y', '-i', $localRawPath];
            $varStreamMap = [];

            // 360p
            $cmd = array_merge($cmd, ['-map', '0:v:0', '-s:v:0', '640x360', '-c:v:0', 'libx264', '-b:v:0', '365k', '-maxrate:v:0', '400k', '-bufsize:v:0', '800k', '-preset', 'veryfast']);
            if ($hasAudio) {
                $cmd = array_merge($cmd, ['-map', '0:a:0', '-c:a:0', 'aac', '-b:a:0', '64k', '-ac', '1', '-ar', '22050']);
                $varStreamMap[] = "v:0,a:0,name:360p";
            } else { $varStreamMap[] = "v:0,name:360p"; }

            // 480p
            $cmd = array_merge($cmd, ['-map', '0:v:0', '-s:v:1', '854x480', '-c:v:1', 'libx264', '-b:v:1', '750k', '-maxrate:v:1', '856k', '-bufsize:v:1', '1200k', '-preset', 'veryfast']);
            if ($hasAudio) {
                $cmd = array_merge($cmd, ['-map', '0:a:0', '-c:a:1', 'aac', '-b:a:1', '96k', '-ac', '2', '-ar', '44100']);
                $varStreamMap[] = "v:1,a:1,name:480p";
            } else { $varStreamMap[] = "v:1,name:480p"; }

            // 720p
            $cmd = array_merge($cmd, ['-map', '0:v:0', '-s:v:2', '1280x720', '-c:v:2', 'libx264', '-b:v:2', '2000k', '-maxrate:v:2', '2200k', '-bufsize:v:2', '3000k', '-preset', 'veryfast']);
            if ($hasAudio) {
                $cmd = array_merge($cmd, ['-map', '0:a:0', '-c:a:2', 'aac', '-b:a:2', '128k', '-ac', '2', '-ar', '44100']);
                $varStreamMap[] = "v:2,a:2,name:720p";
            } else { $varStreamMap[] = "v:2,name:720p"; }

            // 1080p
            $cmd = array_merge($cmd, ['-map', '0:v:0', '-s:v:3', '1920x1080', '-c:v:3', 'libx264', '-b:v:3', '4500k', '-maxrate:v:3', '5000k', '-bufsize:v:3', '7500k', '-preset', 'veryfast']);
            if ($hasAudio) {
                $cmd = array_merge($cmd, ['-map', '0:a:0', '-c:a:3', 'aac', '-b:a:3', '192k', '-ac', '2', '-ar', '44100']);
                $varStreamMap[] = "v:3,a:3,name:1080p";
            } else { $varStreamMap[] = "v:3,name:1080p"; }

            $cmd = array_merge($cmd, [
                '-f', 'hls',
                '-var_stream_map', implode(' ', $varStreamMap),
                '-master_pl_name', 'master.m3u8',
                '-hls_time', '4', // Segmen 4 detik
            ...
            // 5. Upload Hasil ke S3
            $s3Base = 'hls/' . $this->media->id;

            // [UPDATE] Upload Thumbnail (Jika berhasil dibuat)
            if (File::exists($localThumbPath)) {
                Storage::disk('s3')->putFileAs($s3Base, new \Illuminate\Http\File($localThumbPath), 'thumbnail.jpg', 'public');
            }
            ...
            $this->media->update([
                ...
                'thumbnail_path' => $s3Base . '/thumbnail.jpg', // Simpan path thumbnail
                'path_original' => null,
                'duration' => $duration
            ]);
